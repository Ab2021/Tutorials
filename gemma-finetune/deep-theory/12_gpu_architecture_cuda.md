...
## Kernel Design Patterns for Transformers

Understanding how transformer operations map to GPU kernels is key to optimization. Here are the fundamental patterns:

### Compute-Bound vs Memory-Bound Kernels

```
Roofline analysis classifies every kernel:
  ...
  ★ below memory ceiling = memory-bound (most transformer ops!)
  ★ below compute ceiling = compute-bound (matmul)

This is WHY kernel fusion matters so much:
  Fuse pointwise ops → reduce memory traffic → push ops toward compute ceiling
```

### The Three Kernel Families in LLMs

```
1. GEMM Kernels (matrix multiply)
   Used by: Linear projections (Q, K, V, O, MLP layers)
   Implementation: cuBLAS (highly optimized by NVIDIA)
   Tensor Cores: YES → major speedup
   Optimization: Already near peak; torch.compile adds epilogue fusion

2. Elementwise / Pointwise Kernels
   Used by: Activations (GELU, SiLU), residual add, scaling
   Implementation: Triton (generated by torch.compile Inductor)
   Tensor Cores: NO → uses CUDA cores
   Optimization: FUSE EVERYTHING → single kernel for chains of ops

3. Reduction Kernels
   Used by: Softmax, LayerNorm/RMSNorm, loss computation
   Implementation: Triton (generated by torch.compile Inductor)
   Tensor Cores: Partial (norm computations)
   Optimization: Fuse with adjacent pointwise, online algorithms
```

> **For a complete deep dive into kernel design, fusion strategies, custom Triton kernels, and the full torch.compile pipeline from Python bytecode to GPU silicon, see [Guide #15: PyTorch Compile & Kernel Design](./15_torch_compile_and_kernel_design.md).**
